...
public class PhotoBoardDAO {
	...
	public void updatePhotoBoard(PhotoBoardDM photoBoard) {
		...
		try {
			...
			pstmt.setString(3, photoBoard.getFile_path());
			pstmt.setInt(4, photoBoard.getCategory_val());
			pstmt.setString(5, photoBoard.getFile_org_name());
			pstmt.setInt(6, photoBoard.getPassword());
			pstmt.setInt(7, photoBoard.getNo());
			pstmt.executeUpdate();
		}catch (Exception e) {
	...
	public void deletePhotoBoard(int no) {
		...
		try {
			con = ConnectionPool.getConnection();
			StringBuffer sql = new StringBuffer();
			sql.append(" delete from T97_PHOTO where no=? ");
			pstmt = con.prepareStatement(sql.toString());
			pstmt.setInt(1, no);
			pstmt.executeUpdate();
		} catch (Exception e) {
