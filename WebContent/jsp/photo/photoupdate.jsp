...
<div style="margin-top:5px;margin-bottom:5px;text-align: center; height:500px;">
	<c:choose>
		<c:when test="${filepath ne 'check' }">  <!-- 출력, 수정들어갈 시 -->
			<img src="${dm.file_path}" style="width:100%; height: 100%;"  />
			<input type="hidden" name="file_path" value="${file_path}"  />
		</c:when>
		<c:when test="${filepath == 'check' }">  <!--  업로드 시 -->
...
<div class="form-group">
    ...
    <c:choose>
		<c:when test="${filepath ne 'check' }">  <!-- 출력, 수정들어갈 시 -->
			<input type="hidden" name="file_path" value="${dm.file_path}"  />
		</c:when>
		<c:otherwise>
			<input type="hidden" name="file_path" value="${file_path}" />
		</c:otherwise>
	</c:choose>
    <input type="hidden" name="file_org_name" value="${file_org_name}" />
    <input type="hidden" name="no" value="${dm.no}" />
    <input type="text" name="title" class="form-control" id="title"  style="width:100%;" placeholder="제목을 입력해주세요." value="${dm.title}">		
		
		 <p>
		 <label for="content">내용</label>
		 	<input type="text" name="content" id="content" value="${dm.content}" style="padding:20px; width:675px; height:200px;" />
		 </p>
		 <br>
